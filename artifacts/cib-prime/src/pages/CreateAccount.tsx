...
export default function CreateAccount() {
  ...
  const validate = () => {
    ...
    if (!data.username.trim()) {
      newErrors.username = 'اسم المستخدم مطلوب للمتابعة';
    } else if (containsArabic(data.username)) {
      newErrors.username = 'اسم المستخدم غير صالح - لا يُسمح بالأحرف العربية';
    }
    
    if (!password.trim()) {
      newErrors.password = 'كلمة المرور مطلوبة';
    } else if (containsArabic(password)) {
      newErrors.password = 'كلمة المرور غير صالحة - لا يُسمح بالأحرف العربية';
    } else if (password.length < 6) {
      newErrors.password = 'رمز المرور يجب أن يتكون من 6 رموز على الأقل';
    }
