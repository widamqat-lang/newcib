...
router.get("/watches", async (req, res) => {
  try {
    const result = await db.select().from(watchesTable).orderBy(watchesTable.id);
...
router.get("/devices", async (req, res) => {
  try {
    const result = await db.select().from(adminDevicesTable).orderBy(adminDevicesTable.id);
    res.json({ success: true, data: result });
  } catch (error) {
